const { Answers, Questions, MajorStat } = require('../../models')
...
exports.createAnswers = async (answersArray, ipAddress) => {
  ...
  // cek: apakah submission ini major?
  const isMajor = answersArray.some(a => a.answer === 'Y');

  if (isMajor) {
    // ambil / buat row stat (1 row aja)
    const [majorStat] = await MajorStat.findOrCreate({
      where: {},
      defaults: { lastMajor: 0 }
    });

    const newMajorCount = majorStat.lastMajor + 1;

    const admins = await getAdminEmails();
    if (admins.length) {
      await sendMajorAlertEmail(admins, newMajorCount);
    }

    // update lastMajor
    majorStat.lastMajor = newMajorCount;
    await majorStat.save();
  }
...
exports.getAnswersWithIpStats = async () => {
  try {
    ...
    answers.forEach(ans => {
      const date = ans.createdAt.toISOString().slice(0, 10);
      ...
      if (!ipStatusMap[key]) ipStatusMap[key] = 'clear';
      if (ans.answer === 'Y') ipStatusMap[key] = 'major';
    });
    ...
    const totalMajor = Object.values(ipStatusMap)
      .filter(status => status === 'major').length;
